...
class minimizer_ssneb:
    ...
    def minimize(self, forceConverged=0.01, maxIterations=1000):
        ...
        if (not self.band.parallel) or (self.band.parallel and self.band.rank == 0):
            print(
                "Iteration       Total Force       Perp Force        MaxU       MaxI    Stress on CI    ")
            print(
                "---------------------------------------------------------------------------------------")
            feout = open('fe.out', 'a')
            feout.write(
        ...
        while fMax > forceConverged and iterations < maxIterations:
            ...
            for i in range(self.band.numImages):
                if i == 0:
                    ...
                    Rm1 = sPBC(
                        self.band.path[i - 1].vdir - self.band.path[i].vdir)
                    avgb = 0.5 * \
                        (self.band.path[i - 1].get_cell() +
                         self.band.path[i].get_cell())
                    Rm1 = dot(Rm1, avgb)
                    ...
                    if i == self.band.numImages-1:
                        ...
                        realtotalf = vdot(
                            self.band.path[i].realtf, self.band.path[i].n)
                R20 += Rm1
                if (not self.band.parallel) or (self.band.parallel and self.band.rank == 0):
                    mepout.write("%3i %13.6f %13.6f %13.6f %3i %s" % (i, float(R20), float(
                        self.band.path[i].u-self.band.path[0].u), float(realtotalf), i, '\n'))
