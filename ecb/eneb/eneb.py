...
class eneb:
    ...
    def __init__(self, p1, p2, numImages=7, k=5.0, tangent="new",
        ...
        if express[0][1]**2+express[0][2]**2+express[1][2]**2 > 1e-3:
            ...
            if (not self.parallel) or (self.parallel and self.rank == 0):
                print(
                    "warning: xy, xz, yz components of the external pressure will be set to zero")
        self.fixstrain = fixstrain
        self.eneb = eneb
        # electrochemical potential: the work function of the conter electrode under voltage
        ...
        if self.ss:
            for p in [p1, p2]:
                ...
                if cr[0][1]**2+cr[0][2]**2+cr[1][2]**2 > 1e-3:
                    if (not self.parallel) or (self.parallel and self.rank == 0):
                        print(
                            "check the orientation of the cell, make sure a is along x, b is on the x-y plane")
                    sys.exit()
        ...
        for i in [0, n]:
            ...
            if self.eneb:
                ...
                self.path[i].u += (self.path[i].ne-self.ne0) * \
                    (-self.epotential + self.path[i].vtot)
            os.chdir(backfd)
    ...
    def get_mue(self, imgi):
        ...
        vacuumE, vtot_new = align_vacuum(
            direction=self.direction, LOCPOTfile='LOCPOT')

        if self.solPoisson:
            self.path[imgi].vtot = -vacuumE
        else:
            try:
                self.vtot0 += 0  # if vtot0 exists, do nothing
            except:
                self.vtot0 = vtot_new  # save the first vtot_new as vtot0
                # start from zero charge for absolute reference
            self.path[imgi].vtot = (self.vtot0 + vtot_new)*0.5
        self.path[imgi].mue[0][0] = self.epotential - \
            (self.path[imgi]._calc.get_fermi_level() - vacuumE)
        print("image {}, mu of electron: {}, number of electrons: {} ".format(
            imgi, self.path[imgi].mue[0][0], self.path[imgi].ne))
        # print "done."
    ...
    def forces(self):
        ...
        for i in range(1, self.numImages - 1):
            ...
            self.path[i].totalf = numpy.vstack(
                (self.path[i].f, self.path[i].st / self.jacobian))
            # electrochemical
            if self.eneb:
                tmpw = self.eweight if self.eweight else 1.0
                self.path[i].totalf = numpy.vstack(
                    (self.path[i].totalf, self.path[i].mue / tmpw))
            ...
            # saddle points, J. Chem. Phys. 113, 9978-9985 (2000)
                ...
                if (UPm1 != UPp1):
                    if (UPm1):
                        ...
                        dr_dir = sPBC(
                            self.path[i].vdir - self.path[i - 1].vdir)
                        avgbox = 0.5 * \
                            (self.path[i].get_cell() +
                             self.path[i - 1].get_cell())
                        sn = numpy.dot(dr_dir, avgbox)
                        dh = self.path[i].cellt - self.path[i - 1].cellt
                        snb = numpy.dot(
                            self.path[i].icell, dh)*0.5 + numpy.dot(self.path[i - 1].icell, dh)*0.5
                        ...
                        if self.eneb:
                            sne = numpy.zeros((1, 3))
                            sne[0][0] = (self.path[i].ne -
                                         self.path[i - 1].ne) * self.eweight
                            self.path[i].n = numpy.vstack(
                                (self.path[i].n, sne))
                    else:
                        dr_dir = sPBC(
                            self.path[i + 1].vdir - self.path[i].vdir)
                        avgbox = 0.5 * \
                            (self.path[i+1].get_cell() +
                             self.path[i].get_cell())
                        sn = numpy.dot(dr_dir, avgbox)
                        ...
                        if self.eneb:
                            sne = numpy.zeros((1, 3))
                            sne[0][0] = (self.path[i + 1].ne -
                                         self.path[i].ne) * self.eweight
                            self.path[i].n = numpy.vstack(
                                (self.path[i].n, sne))
                # otherwise, we are near some extremum
                    ...
                    if (Um1 > Up1):
                        ...
                        if self.eneb:
                            ...
                            sne1[0][0] = (self.path[i + 1].ne -
                                          self.path[i].ne) * self.eweight
                            sne2 = numpy.zeros((1, 3))
                            sne2[0][0] = (self.path[i].ne -
                                          self.path[i - 1].ne) * self.eweight
        ...
        for i in range(1, self.numImages - 1):
            ...
            if self.method == 'ci' and i == self.Umaxi:
                self.path[i].totalf -= 2.0 * \
                    vproj(self.path[i].totalf, self.path[i].n)
            ...
            # and for the non-climbing images...
                ...
                avgbox = 0.5 * \
                    (self.path[i - 1].get_cell() + self.path[i].get_cell())
                ...
                if self.eneb:
                    ...
                    Rp1e[0][0] = (self.path[i + 1].ne -
                                  self.path[i].ne) * self.eweight
                    Rp1 = numpy.vstack((Rp1, Rp1e))

                self.path[i].fsN = (vmag(Rp1) - vmag(Rm1)) * \
                    self.k * self.path[i].n
                ...
                if self.dneb:
                    ...
                    if not self.dnebOrg:
                        ...
                        if FsperpSQ > 0:
                            self.path[i].fsdneb *= 2.0 / \
                                pi*atan(FperpSQ/FsperpSQ)
